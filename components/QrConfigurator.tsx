...
export default function QrConfigurator({qrs,bikes}:{qrs:any[];bikes:any[]}){
 ...
 useEffect(()=>{if(autoScanRef.current)return;const params=new URLSearchParams(window.location.search);if(params.get("scan")==="1"){autoScanRef.current=true;const t=window.setTimeout(()=>scan(),250);return()=>window.clearTimeout(t)}},[]);
